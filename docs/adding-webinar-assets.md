...
2. Drop in whatever the session used: slides, sample code, scripts. Keep individual files under GitHub's 100 MB limit. Leave the recording out of the repo and link to wherever it's hosted instead. Double-check there are no API keys, tokens, passwords, or other secrets in what you're committing. Sample code and demo scripts are the most common place these slip in. For any placeholder credential in a `.env.example`, curl header, or config sample, use `__REPLACE_ME__` as the value — it's the one placeholder confirmed not to trip the secret scan (see [docs/overriding-a-safe-secret-scan.md](overriding-a-safe-secret-scan.md)). A secret scan runs on every PR as a fallback, but don't rely on it as your only check. If it still flags something you know is safe to publish, like a throwaway self-signed cert the lab regenerates, allowlist it per that same doc instead of merging past the failure.
